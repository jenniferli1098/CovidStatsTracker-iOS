...
// *************************************** New Stuff ********************************************** //
    ...
    func loadFromLocalFile() {
        let filePath = Bundle.main.path(forResource: "innovators", ofType: "csv")
        let str = try? String.init(contentsOfFile: filePath!, encoding: .utf8)
        let items: [(city: String, region: String, country: String)] = parseCsvString(csvString: str!)
        
        let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
        
        for item in items {
            let location = Cities(context: context)
            location.cityName = item.city
            location.regionName = item.region
            location.countryName = item.country
